 ...
 * @returns http server
 */
export async function createGraphqlServer(deps, { port, host } = {}) {
    const app = express();
    ...
    return new Promise((resolve) => httpServer.listen({ port: serverPort }, () => {
        logger.info(`🚀 Server ready at http://${serverHost}:${serverPort}/graphql`);
        resolve(httpServer);
    }));
}
